...
Large-archive listing and search now use the derived session index, including dirty-entry invalidation and Bloom-filter candidate pruning. The next architecture improvement should focus on API pagination/conditional responses and, if archive sizes eventually justify it, evaluating SQLite/FTS as an optional storage backend while retaining JSON import/export. A TypeScript migration can still be considered later, but it is no longer required to express the current module boundaries.

The current test suite covers large-archive index invalidation/search pruning, server API/storage behavior, crash-recovery injection for multi-file mutations, security boundaries, concurrent writes, idempotency, markdown rendering, web UI API/render/controller/event seams, a browser-level jsdom flow through the real web UI runtime, native ES module entrypoint loading, extension background API calls, provider-adapter resolution, content-script DOM fixtures plus mutation-resistance and diagnostics for provider extraction/injection, manual clipboard/message extraction, autosave idempotency/dedupe behavior, sidebar fixture behavior, composer/load-past modal behavior, runtime behavior for auto-send toggles, local-app availability, Save local delegation, dependency-free Electron navigation/security/shutdown helpers, a packaged-Electron launch smoke test, web UI accessibility contracts, modal focus behavior, and browser-level accessibility assertions.
