...
- The frontend is intentionally vanilla JavaScript and now uses a single native ES module entrypoint at `public/app/main.mjs`. Browser code is split into `api.mjs`, `state.mjs`, `render.mjs`, `message-navigator.mjs`, `modals.mjs`, `clipboard.mjs`, `events.mjs`, `export.mjs`, and `markdown.mjs` without a bundler. `controllers.mjs` is now only a controller composition root; domain behavior lives under `public/app/controllers/` in session, folder, message, and active-session modules.
